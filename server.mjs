...
// Loads a local .env file for local development. Render (and most hosts) inject env vars
// directly, so there's no .env file there and this safely does nothing.
// process.loadEnvFile is a fairly new Node API — accessed dynamically (not statically
// imported) so this file doesn't crash at startup on older Node runtimes that lack it.
